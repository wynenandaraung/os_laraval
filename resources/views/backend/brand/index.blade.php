			...
			<table class="table table-border">
				...
				<tbody>
					...
					<tr>
						...
						<td><form action="{{route('brands.destroy',$brand->id)}}" method="post" onsubmit="return confirm('Are you Sure want to Delete!')" class="d-inline-block">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-danger">Delete</button>
                                    </form>
						<a href="{{ route('brands.edit',$brand->id) }}" class="btn btn-primary">Edit</a>
					<a href="{{route('brands.show',$brand->id)}}" class="btn btn-primary">Detail</a></td>
					</tr>
					@endforeach
